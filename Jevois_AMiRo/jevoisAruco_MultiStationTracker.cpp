...
int station_idx = 0;
int desired_Marker_ID_Stations[] = {1,2,3,4};
int desired_Marker_ID;
...
double P_linear = 500;
double P_angular = 3000;
double disired_dist = 200;
    ...
    while (1)
    {
        ...
        if (buf == '\n')
        {
            ...
            if(marker_found == true) {
                ...
                if(marker_tracked == false){
                    //printf("Marker_ID: %d\n",DecodedMarker.id);
                if (actual_ID == desired_Marker_ID && (state == SEARCH || state ==  START_SEARCH_TURN || state == SEARCH_TURN)) {
        ...
        case SEARCH:
        {
            if (actual_ID == desired_Marker_ID)
            {
                ...
                nextState = TURN_MIDDLE_TO_TARGET;
            }
            else
            {
        ...
        case SEARCH_TURN:
        {
            if (actual_ID == desired_Marker_ID)
            {
                printf("State=TURN_MIDDLE_TO_TARGET\n");
                nextState = TURN_MIDDLE_TO_TARGET;
                Turning_Done = false;
            }
        ...
        case SEARCH_TURN:
        {
            ...
            // if (nextState == DRIVE_MIDDLE_TO_TARGET) {
            //     CAN.setTargetSpeed(0,0);
            //     usleep(500000);
            // }
            break;
        ...
        case TURN_MIDDLE_TO_TARGET:
        {
            ...
            CAN.setTargetSpeed(0, angular_speed);
            break;
        }
        case DRIVE_MIDDLE_TO_TARGET:
        {           
