...
class OpticalFlow : public jevois::Module
{
  public:
    ...
    virtual void process(jevois::InputFrame && inframe) override
    {
	  ...
	  // Output matrix same width as input, 2x taller, and grey pixels
      cv::Mat cvout(h*2, w, CV_8UC1);
      itsOpticalFlow->process(cvimg, cvout);     

	  // Compute relative nearness map
	  double nearnessMap[w];
	  for(int col = 0; col < w; col++) {
		nearnessMap[col] = 0;
	  	for(int row = 0; row < h; row++) {
			// pixels - 128: 255 pixels all together => 1st 128: optical flow on left side, 2nd 128: optical flow on right side
			nearnessMap[col] += sqrt(pow(cvout.at<uint8_t>(row, col) - 128, 2) + pow(cvout.at<uint8_t>(row + h, col) - 128, 2));
		}
	    nearnessMap[col] /= h;
	  }

	  // Azimuth angle = col, no elevation angle
	  // Compute COMANV = vector pointing towards closest object
	  double COMANV[2] = {0, 0};
	  double calculation = 0;
	  for(int col = 0; col < w; col++) {
		  calculation = (col - 88.0f) * 65.0f / 88.0f * PI / 180.0f;
		  COMANV[0] += cos(calculation) * nearnessMap[col]; //angle of 65 degrees, but 88 pixels per side
		  COMANV[1] += sin(calculation) * nearnessMap[col];   
	  }	
	
      // Send the output matrix over USB:
      this->sendSerial("OF," + std::to_string(COMANV[0]) + "," + std::to_string(COMANV[1]));
    }
